...
export default function AgentPage() {
  ...
  const fetchReport = async (claimId: string) => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/damage/report/${claimId}`,
      );
      const data = await res.json();
  ...
  const fetchPhotos = async (claimId: string) => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/damage/photos/${claimId}`,
      );
      const data = await res.json();
  ...
  const updateStatus = async (claimId: string, status: string) => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/claims/${claimId}/status`,
        {
          method: "PUT",
          headers: {
  ...
  const statusColor = (status: string) => {
    switch (status) {
      case "PENDING":
        return "bg-yellow-500 text-yellow-900 font-bold";
      case "UNDER_REVIEW":
        ...
        <div className="bg-slate-800 rounded-2xl p-10 w-full max-w-sm shadow-2xl border border-slate-700">
          <div className="text-center mb-8">
            ...
            <p className="text-slate-400 text-sm">
              Agent Portal — Restricted Access
            </p>
          </div>
            ...
            filteredClaims.map((claim) => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full ${statusColor(claim.status)}`}
                  >
                    {claim.status}
                  </span>
                </div>
            ...
            <div className="max-w-2xl space-y-4">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-slate-400 text-sm">
                    {selected.location} · {formatDate(selected.incident_date)}
                  </p>
                </div>
                <span
                  className={`text-xs px-3 py-1.5 rounded-full ${statusColor(selected.status)}`}
                >
                  {selected.status}
                </span>
              </div>
              ...
              {report ? (
                <>
                  ...
                  <div className="card">
                    ...
                    <div className="flex items-center gap-3 mb-4">
                      <span
                        className={`${severityColor(report.severity)} text-xs font-bold px-3 py-1.5 rounded-full`}
                      >
                  ...
                  <div className="card">
                    ...
                    <div className="flex items-end gap-3">
                      <p className="text-4xl font-bold text-white">
                        ${report.estimated_cost_min.toLocaleString("en-US")}
                      </p>
                      <p className="text-slate-400 text-xl mb-1">
                        — ${report.estimated_cost_max.toLocaleString("en-US")}
                      </p>
                    </div>
                  </div>
                </>
                ...
                <div className="card">
                  <p className="text-slate-400 text-sm">
                    No damage report yet for this claim.
                  </p>
                </div>
              )}
          ...
          >
            ...
            <img
              ...
              onWheel={(e) => {
                ...
                const newScale =
                  e.deltaY < 0
                    ? Math.min(currentScale + 0.15, 4)
                    : Math.max(currentScale - 0.15, 0.5);
